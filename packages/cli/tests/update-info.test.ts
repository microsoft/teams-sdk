...
import { compareVersions, isNewerVersion } from '../src/utils/update-info.js';
...
describe('version comparison', () => {
  ...
  it('compares prerelease versions numerically', () => {
    expect(compareVersions('3.0.0-beta.10', '3.0.0-beta.2')).toBeGreaterThan(0);
    expect(compareVersions('3.0.0-beta.2', '3.0.0-beta.10')).toBeLessThan(0);
  });

  it('treats stable releases as newer than prereleases with the same core version', () => {
    expect(compareVersions('3.0.0', '3.0.0-beta.10')).toBeGreaterThan(0);
    expect(compareVersions('3.0.0-beta.10', '3.0.0')).toBeLessThan(0);
  });
