...
export function buildHierarchicalStructure(rootPath: string): { [key: string]: FolderStructure } {
  ...
  function processDirectory(
    dirPath: string,
    currentLevel: { files: FileInfo[]; children: { [key: string]: FolderStructure } }
  ): void {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory() && !shouldSkipDirectory(item.name)) {
        ...
        const indexFilePath = fs.existsSync(readmePath)
          ? readmePath
        ...
        try {
          ...
          if (
            frontmatter.llms === 'ignore' ||
            frontmatter.llms === 'ignore-file' ||
  ...
  const tempWrapper: { files: FileInfo[]; children: { [key: string]: FolderStructure } } = {
    files: [],
    children: {},
  };
...
export function getPriorityFiles(organized: any): string[] {
  ...
  const keyTeamFiles = organized.main.teams.filter(
    (file: string) => file.includes('core-concepts') || file.includes('README.md')
  );
  ...
  const essentialReadmes = organized.language.essentials.filter(
    (file: string) => file.includes('README.md') || file.includes('app-basics')
  );
