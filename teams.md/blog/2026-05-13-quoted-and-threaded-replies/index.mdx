...
Both features have been on agents' wish lists for a while. Two things changed.
...
For threaded replies, threading in channels has been supported on the service for a long time, but the SDK didn't have a helper for sending into a thread when there was no inbound activity in hand. Threading in chats and meetings is on the service roadmap, and we wanted a proactive thread-send API to be available everywhere ahead of those expansions. `app.reply(conversationId, rootMessageId, ...)` is that API.
...
This is the most common case, and it's free. No extra arguments, no extra builders. See [Reactive auto-quote](#reactive-auto-quote-contextreply) for the code in each SDK.

### 2. Posting an Async Result Back Into Its Thread

A user kicks off a deployment with a slash command. Your agent acknowledges, then forty minutes later the deployment finishes. If the result lands at the top level of the conversation (not nested under a parent message), nobody can tell which deployment it's about. If it lands in the thread of the original request, it shows up grouped with the command that started it, collapsed under the same root.

Threading already exists on the service for channels, and is coming to chats and meetings. The gap the SDK is filling here is the proactive case. `app.reply(conversationId, rootMessageId, ...)` lets you post into the thread of the original request when there's no inbound activity to react to. You store the IDs when the request comes in, then post back into the same thread when the work completes. See [Proactive thread send](#proactive-thread-send-appreply) for the proactive call, and [Extracting the thread root from an inbound activity](#extracting-the-thread-root-from-an-inbound-activity) for how to pull the IDs you'll need to stash.
...
See [Multi-quote builder](#multi-quote-builder) for the equivalent code in .NET and Python, and [Proactive thread send](#proactive-thread-send-appreply) for the `app.reply()` call that lands the digest in a thread.
...
This is the inbound side of quoting, and reading it is part of being a good conversational participant. As agents become more sophisticated about context, picking up the messages a user has explicitly pointed at is one of the simpler wins. See [Reading inbound quotes](#reading-inbound-quotes) for the code in each SDK.

## Quoted Replies

<div style={{textAlign: 'center'}}>
<img src={require('./quoted-reply-bot-response.png').default} alt="A user message and the agent's quoted reply in a Teams chat: the agent's response carries a grey quote bubble with the original message above the reply text, so the answer stays anchored to the question." style={{maxHeight: '400px'}} />
</div>

*Above: the agent's reply (shown without the surrounding chat) carries a grey quote bubble. The bubble holds the original message; the bot's response follows below.*
