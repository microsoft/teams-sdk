...
| Build a message with one or more quotes | `MessageActivity.addQuote()` | `MessageActivity.AddQuote()` | `MessageActivityInput.add_quote()` |
| Read quoted-reply metadata from an inbound message | `activity.getQuotedMessages()` | `Activity.GetQuotedMessages()` | `activity.get_quoted_messages()` |
| Send proactively into a thread | `app.reply()` | `teams.Reply()` | `app.reply()` |
| Construct a threaded conversation ID by hand | `toThreadedConversationId()` | `Conversation.ToThreadedConversationId()` | `to_threaded_conversation_id()` |
