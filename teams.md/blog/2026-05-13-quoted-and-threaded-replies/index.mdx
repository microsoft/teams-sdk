...
2. **The misplaced async result.** A background job finishes and posts at the top of the conversation. Whoever asked has to scroll up to find the request. Whoever else in the conversation sees a result with no context.
...
Two operations don't tie neatly to one of the scenarios above but are worth knowing about.
...
If you want to build the threaded conversation ID yourself (for example, to send a fully-customized activity via the generic `app.send()` instead of a string through `app.reply()`), the helper is exposed.
...
- Proactive messaging: [TypeScript](https://microsoft.github.io/teams-sdk/typescript/essentials/sending-messages/proactive-messaging/) · [.NET](https://microsoft.github.io/teams-sdk/csharp/essentials/sending-messages/proactive-messaging/) · [Python](https://microsoft.github.io/teams-sdk/python/essentials/sending-messages/proactive-messaging/)
