...
async def handle_message(ctx: ActivityContext[MessageActivity]):
    ...
    reply = MessageActivityInput().add_ai_generated() # Add AI generated label
...
async def handle_message(ctx: ActivityContext[MessageActivity]):
    ...
    reply = MessageActivityInput().add_ai_generated() 
            .add_feedback(mode="custom") # Add feedback handling
    ctx.stream.emit(reply)
```
...
async def handle_message(ctx: ActivityContext[MessageActivity]):
    ...
    reply = MessageActivityInput().add_ai_generated() 
            ...
            .with_suggested_actions( # Add suggested actions
...
async def handle_message(ctx: ActivityContext[MessageActivity]):
    ...
    citations = tool_logger.get_citations() # fetch citations from agent middleware
    attach_citations(reply, full_text, citations) # attach citations to reply
...
def attach_citations(reply, full_text, citations):
    ...
    for c in citations:
        # filter for citations that are actually referenced in final text
        if c.position in used:
            # Add each citation as an entity to the reply activity
            reply.add_citation( 
                position=c.position,
