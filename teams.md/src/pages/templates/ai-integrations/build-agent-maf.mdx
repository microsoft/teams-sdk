...
The Teams integration connects incoming messages to the agent runtime and streams the response back to the chat interface.
```python
@app.on_message
...
async def handle_message(ctx: ActivityContext[MessageActivity]):
    ...
    async for chunk in agent.run(ctx.activity.text or "", session=session, stream=True):
        ...
```
In production push sessions into Redis, Cosmos DB, or whatever you already use for state. Watch the token budget too — long sessions can blow past the model's context window.
