...
| **Quickstart** | New projects created via Teams CLI, Visual Studio extension, or VS Code extension. | New projects created via Slack CLI. |
| **App manifest** | Authored via JSON, Teams CLI, and/or via Teams Developer Portal. Must also setup Azure Bot and Azure App Registration resources, which can be done via Azure Portal or Teams CLI. | Authored via JSON, YAML, or app management page. |
...
First, setup a new Teams application, as shown in [Teams Platform](/teams).
...
First, follow the instructions in the [Teams SSO guide](/teams/user-authentication/sso-setup).
