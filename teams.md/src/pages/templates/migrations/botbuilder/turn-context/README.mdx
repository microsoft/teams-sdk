---
title: Turn Context
sidebar_position: 3
languages: ['typescript', 'csharp', 'python']
summary: Understand how Teams SDK's IActivityContext replaces BotBuilder's TurnContext for handling incoming data and APIs.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Turn Context

While BotBuilder has a `TurnContext`, Teams SDK has <LanguageInclude content={{"typescript": "`IActivityContext`", "csharp": "`IActivityContext`", "python": "`ActivityContext`"}} /> which serves the same purpose.
The context is passed into activity handlers and provides a structured way for developers to interface
with apis and incoming data.
