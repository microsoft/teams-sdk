...
For sovereign cloud or non-public endpoints, add the extra environment variables:
...
## 4. Agentic identity (new in SDK 2.1)

SDK 2.1 supports running your bot as an **AI teammate** in Agent 365 — a first-class Microsoft 365 identity with its own presence, mailbox, and directory entry. No extra configuration is needed; the SDK resolves the correct token per turn automatically.

Use `GetAgenticIdentity()` on the activity recipient to detect and branch on agentic context:
...
teams.OnMessage(async (context, cancellationToken) =>
{
    ...
    {
        // Running as an AI teammate — APIs are automatically scoped to its identity
...
See the [Agentic Identity guide](../in-depth-guides/agentic-identity) for the full setup.

## 5. Update helper calls to SDK 2.1 APIs
...
## 7. Add built-in state management
...
OpenTelemetry support is new in SDK 2.1 — there was no equivalent in 2.0. The SDK instruments its pipeline via standard .NET primitives (`ActivitySource`, `Meter`, `ILogger`). Your app opts in by registering the SDK's source and meter names and choosing an exporter.
...
builder.Services.AddOpenTelemetry()
    .WithTracing(tracing => tracing
        .AddSource(TeamsTelemetry.ActivitySourceName)
        .AddAzureMonitorTraceExporter())
    .WithMetrics(metrics => metrics
        .AddMeter(TeamsTelemetry.MeterName)
        .AddAzureMonitorMetricExporter());
```

See the [OpenTelemetry guide](../in-depth-guides/observability/opentelemetry) for the full setup including exporters, sampling, and correlated logs.

## 12. Message extensions, task modules, files, and meetings
...
## 13. Plugin/event-bus patterns → turn middleware
...
## 14. Update local dev auth behavior
