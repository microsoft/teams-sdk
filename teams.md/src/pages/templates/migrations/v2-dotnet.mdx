...
| Message send APIs | `context.Send(...)` | `context.SendAsync(...)` |
...
## 2. Configuration key map (SDK 2.0 → SDK 2.1)

SDK 2.1 uses standard ASP.NET Core configuration sections (`AzureAd`, `BotFramework`) instead of SDK 2.0 `Teams:*` settings.

| SDK 2.0 key | SDK 2.1 key | Notes |
|---|---|---|
| `Teams:ClientId` | `AzureAd:ClientId` | Bot app/client ID |
| `Teams:TenantId` | `AzureAd:TenantId` | Entra tenant ID |
| `Teams:ClientSecret` | `AzureAd:ClientSecret` | Only for secret-based auth |
| `Teams:Cloud` | `AzureAd:Instance` + `BotFramework:*` endpoints | In 2.1, set explicit cloud endpoints |
| `Teams:LoginTenant` | `AzureAd:TenantId` | Use tenant-scoped config directly |
...
Replace app/context helper method usage:

- `context.Send(...)` → `context.SendAsync(...)`
- `context.Typing(...)` → `context.TypingAsync(...)`
