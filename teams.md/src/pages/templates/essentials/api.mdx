...
In this example, we use the API client to get a meeting participant's details. The `meetingId` and `tenantId` are available from the activity's channel data, and the `participantId` is the user's AAD Object ID.
