...
Teams has a number of areas that your application has access to via its API. These are all available via the <LanguageInclude content={{"typescript": "`app.api`", "csharp": "`context.Api` (and `teams.Api` at the app level)", "python": "`app.api`"}} /> object. Here is a short summary of the different areas:
...
In this example, we use the API client to fetch the members in a conversation. The <LanguageInclude content={{"typescript": "`api`", "csharp": "`Api`", "python": "`api`"}} /> object is passed to the activity handler in this case.
