...
Teams keeps a streamed message open for two minutes, measured from its first chunk. After that the service rejects further chunks with a `403` and the [error code](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/streaming-ux#error-codes) `ContentStreamNotAllowed` (`Content stream finished due to exceeded streaming time.`).
...
Plain updates are throttled more aggressively than streamed chunks, so it is recommended to send updates in 1-2 second increments, as opposed to by tokens.
