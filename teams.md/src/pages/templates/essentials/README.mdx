...
flowchart LR
    Teams["Teams"]
    Server["App Server"]
    AppEventHandlers["<LanguageInclude content={{"typescript": "Event Handler (app.event())", "csharp": "Activity Handling (middleware)", "python": "Event Handler decorator (@app.event())"}} />"]
    AppRouter["Activity Event Router"]
    AppActivityHandlers["<LanguageInclude content={{"typescript": "Activity Handlers (app.on())", "csharp": "Activity Handlers (OnMessage, ...)", "python": "Activity Handler decorators (@app.on_activity())"}} />"]
