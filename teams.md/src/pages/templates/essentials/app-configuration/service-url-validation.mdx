...
If you have a non-standard setup where domain-based validation does not work, you can disable it by passing `['*']` as the sole entry. This is the only wildcard form supported and is interpreted as "disable validation"; mixing `*` with other entries has no additional effect beyond the disable.
