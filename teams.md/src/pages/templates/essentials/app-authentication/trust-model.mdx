...
<LanguageInclude section="custom-route-shared-secret-example" />

If the route needs the same trust posture as `/api/messages` — Microsoft-issued bearer tokens rather than a shared secret — reuse the SDK's built-in validator instead. See the [App Authentication](/teams/app-authentication) guide for how to compose validators.
