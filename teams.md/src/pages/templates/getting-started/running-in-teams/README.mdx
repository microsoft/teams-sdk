...
summary: Guide to deploying and testing your locally running agent in Microsoft Teams using the Teams CLI or Microsoft 365 Agents Toolkit.
llms: ignore
---

# Running In Teams

Now that you completed [Local Setup](../quickstart) and your agent is running locally, let's deploy it to Microsoft Teams for testing.

There are two ways to get your agent into Teams:

| | [Teams CLI](/get-started/quickstart-register) | Microsoft 365 Agents Toolkit |
|---|---|---|
| **Best for** | Quick sideload, CI/CD, no IDE required | VS Code debugging with full provisioning |
| **How it works** | `teams app create` + `teams app manifest upload` | VS Code extension handles tunnel, manifest, and sideload |
| **Azure required** | No (Teams-managed) or Yes (Azure bot) | No (local dev uses devtunnel + M365) |

The rest of this guide covers the **Agents Toolkit** path. For the CLI path, see [Quickstart: Register your app](/get-started/quickstart-register).
