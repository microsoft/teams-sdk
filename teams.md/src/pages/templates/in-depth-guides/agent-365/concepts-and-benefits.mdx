...
`AgenticIdentity` is an SDK type, not another platform identity. Use it to scope proactive sends, API clients, and telemetry. Its shape determines how narrowly the operation is scoped.

The blueprint ID is required for every scope. Agentic App Instance and Agentic User IDs are optional in the SDK type because not every identity payload includes them. The tenant ID is required when an operation acquires a token.
