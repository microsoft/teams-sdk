...
:::note Migrate first if your bot is Teams-managed
SSO requires an Azure-managed bot because the OAuth connection lives in Azure Bot Service. If you registered with `teams app create` (default Teams-managed), upgrade your bot first:

```bash
teams app bot migrate <appId> --resource-group <rg>
```

Your `CLIENT_ID`, `CLIENT_SECRET`, and `TENANT_ID` stay the same. See [Bot Locations](/cli/concepts/bot-locations) for the migration details.
:::
...
If you'd rather script it yourself, see [User Authentication Setup](/cli/guides/user-authentication-setup) for the full sequence of `az` and Graph API calls.
