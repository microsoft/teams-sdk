...
Create a file called `patch1.json` with the following content, substituting your `appId` and `scopeId`:
...
Pre-authorize the Teams desktop and web clients so they can silently acquire SSO tokens. Create `patch2.json`, substituting your `scopeId`:
...
:::note
PATCH 2 must run after PATCH 1 because the pre-authorized apps reference the `scopeId`, which must exist first. If it fails with a scope-not-found error, wait 15 seconds (AAD replication lag) and retry.
:::
...
The full SSO setup involves several steps the Teams CLI doesn't handle directly (creating the `access_as_user` scope, pre-authorizing Teams clients, configuring the OAuth connection in Azure Bot Service, updating `webApplicationInfo` in the manifest). The [`teams-dev` skill](/cli/getting-started/agent-skills) orchestrates all of this for you — install it in your AI coding assistant and say *"set up SSO for my Teams bot"*.
...
Use the `az bot authsetting create` command to create the OAuth connection. Replace the placeholders with your values:
...
:::tip
`User.Read` covers basic user profile. To include additional Graph permissions, space-delimit them: `"User.Read Mail.Read"`. For SharePoint or custom APIs, server-side On-Behalf-Of (OBO) token exchange in bot code is required.
:::
...
The Teams application manifest needs to be updated to reflect the settings configured above, with the `Application Id` and `Application ID URI`. If not using `devtunnels`, replace the valid domain with the domain hosting your application.

<Tabs groupId="sso-setup">
<TabItem value="portal" label="Manifest JSON">

Edit your `manifest.json` file to include the following:
