...
### Configure with the Teams CLI

The Teams CLI automates this entire flow (Entra app config, OAuth connection, manifest update) end-to-end. See [User Authentication Setup](/cli/guides/user-authentication-setup) for the full scripted setup, including the `oauth2PermissionScopes` and `preAuthorizedApplications` that the manual `az ad app update` commands above don't cover.
